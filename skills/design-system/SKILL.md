...
3. Poll for feedback with the documented long-poll form, again using the artifact's canonical absolute path in actual use: `npx -y lavish-axi poll /tmp/lavish-probe/test.html`. Leave it running; it is expected to stay silent until feedback arrives or the session ends. If the harness kills the poll, re-run it because queued feedback is not lost.
4. On feedback, apply the requested changes to the same artifact path and poll again with the documented `--agent-reply` workflow from `docs/lavish-axi-cli.md`, using the same `npx -y lavish-axi poll /tmp/lavish-probe/test.html` command form. End the loop when the human accepts, requests tmux-only feedback, or ends the session.
