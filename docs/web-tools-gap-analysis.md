...
- **Option B (cloud):** Call Parallel Extract API directly. No Playwright binary needed; handles JS + PDF automatically. Costs $0.01/request. Affected files: add `src/extensions/websearch/parallel.ts`, new tool registration in `tools.ts`.
...
### Priority 3: `objective` parameter on `web_search`, upgrade to Parallel Search API (larger effort, biggest quality leap)

Migrate search from Tavily to Parallel Search API to unlock: objective framing, multi-query fan-out, advanced mode, publish dates, and freshness control.

**What to build:**
- Replace `src/extensions/websearch/search.ts` internals with a Parallel Search call (POST `https://api.parallel.ai/v1/search`). The external `searchWeb()` signature can stay stable; only the request body and response mapping changes.
- Update `types.ts`: add `PARALLEL_API_KEY_ENV`, add `publishDate?: string` to `SearchResult`.
- Update `tools.ts`: replace `TAVILY_API_KEY_ENV` check with `PARALLEL_API_KEY_ENV`; add optional `resultCount`, `mode` (basic/advanced), `includeDomains`, `excludeDomains` tool params.
- The `SKILL.md` description of "Requires `TAVILY_API_KEY`" must be updated.
- **Files affected:** `search.ts`, `types.ts`, `tools.ts`, `skills/web-tools/SKILL.md`.
- **Complexity:** Large if fully replacing Tavily (new response mapping, new types, updated tests); Medium if keeping Tavily as fallback and adding Parallel Search as a separate code path.
...
- **What it is:** Browserbase, Bright Data Scraping Browser, Apify. Cloud-hosted headless browsers accessible via REST or WebSocket. Handle anti-bot measures, proxies, and session management.
- **Pros:** Solves bot-detection problems (Cloudflare, reCAPTCHA) that local Playwright cannot; some offer geolocation rotation.
- **Cons:** Cost per session, vendor lock-in, overkill for internal use.
- **Recommendation:** Only relevant if the target sites actively block headless browsers. Not needed for the primary use case of fetching technical documentation and SaaS pages.

### Recommendation

**For JS rendering of public pages:** Add a `web_browse` tool backed by **Playwright** (Option A). This is a local, free, reliable solution that eliminates the SPA blank-page problem. It does not require a new API key or cloud dependency.
